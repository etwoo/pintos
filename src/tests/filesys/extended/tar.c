...
usage(void)
{
	printf("tar, tar archive creator\n"
	       ...
	       "  and FILE... is a list of files or directories to put into "
	       "it.\n"
	       "(ARCHIVE itself will not be included in the archive, even if "
	       "it\n"
...
static bool
archive_ordinary_file(const char *file_name,
                      int file_fd,
                      int archive_fd,
                      bool *write_error)
{
...
static bool
archive_directory(char file_name[],
                  size_t file_name_size,
                  int file_fd,
                  int archive_fd,
                  bool *write_error)
