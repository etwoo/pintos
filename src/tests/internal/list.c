...
static bool
value_less(const struct list_elem *, const struct list_elem *, void *);
...
test(void)
{
	...
	for (size = 0; size < MAX_SIZE; size++) {
		...
		for (repeat = 0; repeat < 10; repeat++) {
			...
			ASSERT(size ? list_entry(e, struct value, elem)
			                               ->value == 0
			            : e == list_begin(&list));
...
/* Returns true if value A is less than value B, false
   otherwise. */
static bool
value_less(const struct list_elem *a_,
