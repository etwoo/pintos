...
static void read_partition_table(struct block *,
                                 block_sector_t sector,
                                 block_sector_t primary_extended_sector,
                                 int *part_nr);
static void found_partition(struct block *,
                            uint8_t type,
...
static void
read_partition_table(struct block *block,
                     block_sector_t sector,
                     block_sector_t primary_extended_sector,
                     int *part_nr)
{
	...
	struct partition_table_entry {
		...
		uint8_t start_chs[3]; /* Encoded starting cylinder, head,
		                         sector. */
	...
	for (i = 0; i < sizeof pt->partitions / sizeof *pt->partitions; i++) {
		           ...
		           || e->type == 0xc5) /* DR-DOS extended partition. */
		{
			...
			/* The interpretation of the offset field for extended
			   ...
			   the device's primary partition table in sector 0,
			   then the offset is an absolute sector number.
			   Otherwise, no matter how deep the partition table
			   we're reading is nested, the offset is relative to
			   the start of the extended partition that the MBR
			   points to. */
			if (sector == 0)
...
static void
found_partition(struct block *block,
                uint8_t part_type,
                block_sector_t start,
                block_sector_t size,
                int part_nr)
